...
class DateFTClass extends BaseFTClass {
	...
	function CreateContent($field, $row, $name, $params) {
		...
		if (isset($this->dtconf->dtf[$dtName][$name]["show"]) and $this->dtconf->dtf[$dtName][$name]["show"] == "selects") {
			$array = array("year" => "Y", "month" => "m", "date" => "d");
			foreach ($array as $idx => $val) {
				$field->setAttribute($idx, date($val, $d));
			}
		} elseif ($d > 0) {
			$format = isset($this->dtconf->dtf[$dtName][$name]["view"]) ? $this->dtconf->dtf[$dtName][$name]["view"] : false;
	...
	function AdditionalDeftContent($field, $name, $dtName, $showHidden) {
		...
		if (isset($this->dtconf->dtf[$dtName][$name]["show"]) and $this->dtconf->dtf[$dtName][$name]["show"] == "selects") {
			...
			require_once(CMSPATH_LIB . "datetime/date.php");
			$nodeset = new DateClass($this->xml, $field); 
		} else {
			$format = "Y-m-d";
