...
FOR %%x IN ("D:\htdocs\phpcms5\bin\lib\pear\pear") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=D:\htdocs\phpcms5\bin\lib\pear\pear"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=D:\htdocs\phpcms5\bin\lib\pear"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=d:\php\php.exe"
GOTO :INSTALLED
